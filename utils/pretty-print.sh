...
function __base_log() {
    ...
    [ -z "$WB_SILENT" ] && [ "${WB_LOG_LV:-5}" -ge "$level" ] || return
...
function danger() { __base_log -l 2 "\e[1;33m🍊" "$@" ; }

function error() { __base_log -l 1 "\e[1;31m🍓" "$@" ; }

function critical() { __base_log -l 0 "\e[1;41m💥💥💥" "$@" ; }
